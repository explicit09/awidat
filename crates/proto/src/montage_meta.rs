//! Strongly-typed `metadata.montage` namespace.
//!
//! Per `PLAN.md` §3, the only schema extension Montage makes to OTIO is the
//! `metadata.montage` block. We model it strongly (NOT as `serde_json::Value`)
//! so unknown fields surface as parse errors against the schema we own.
//!
//! Three locations of montage metadata exist:
//!
//! - On a [`crate::otio::Timeline`]'s `metadata`: see
//!   [`MontageTimelineMetadata`].
//! - On a [`crate::otio::Clip`]'s `metadata`: see [`MontageClipMetadata`].
//! - On a [`crate::otio::Marker`]'s `metadata`: see
//!   [`MontageMarkerMetadata`].
...
/// Top-level montage metadata on a `Timeline.metadata.montage`.
///
...
pub struct MontageTimelineMetadata {
    /// Montage project format version, e.g. `"0.1"`.
    #[serde(default)]
    pub version: String,
    ...
    /// Montage-owned transcript alignment packages derived from transcript sidecars.
    #[serde(default)]
...
/// Durable episode span metadata stored under `Timeline.metadata.montage`.
...
/// Per-clip montage metadata on a `Clip.metadata.montage`.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MontageClipMetadata {
    ...
    /// Per-clip anchor — same shape as in [`MontageTimelineMetadata::anchors`]
...
/// Per-marker montage metadata on a `Marker.metadata.montage`.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MontageMarkerMetadata {
...
mod tests {
    ...
    fn timeline_metadata_roundtrip() {
        let mut m = MontageTimelineMetadata {
            version: "0.1".into(),
    ...
    fn unknown_fields_round_trip_through_extra() {
        // A future engine adds a `taste_profile_id` field to the montage
        ...
        let m: MontageTimelineMetadata = serde_json::from_value(json).unwrap();
        assert_eq!(
