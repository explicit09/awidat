use montage_eval::{PacingStats, gates};
...
fn cold_open_passes_a_front_loaded_blitz() {
    ...
    let report = gates::cold_open(&stats(&cuts, 1800.0));
    assert!(report.passed, "{}", report.detail);
}
...
fn cold_open_fails_when_the_peak_is_buried_mid_show() {
    ...
    let report = gates::cold_open(&stats(&cuts, 3274.0));
    assert!(!report.passed);
}

#[test]
fn body_pacing_flags_rates_outside_the_archetype_band() {
    // Emotional-archetype band from the study: 2.0–9.0 cuts/min in the body.
    // 1 cut/min body is under the floor.
    let cuts: Vec<f64> = (2..30).map(|i| f64::from(i) * 60.0).collect();
    let report = gates::body_pacing(&stats(&cuts, 1800.0), 2.0, 9.0);
    assert!(!report.passed);
