...
fn main() {
    if let Err(err) = real_main() {
        eprintln!("montage-index-perf: {err}");
        std::process::exit(1);
    }
}
...
fn apply_work_env(servers: &mut [montage_config::McpServer], work_dirs: &WorkDirs) {
