//! `plan_emphasis` agent tool.
//!
//! Read-only planner for single-clip emphasis motion. It is the clip-level
//! peer to `plan_transition`: choose one executable parameter animation,
//! explain why, and return an EDL fragment the agent can pass to `apply_edl`.
...
/// Read-only emphasis-motion planner.
pub struct PlanEmphasisTool;

#[derive(Debug, Deserialize)]
struct PlanEmphasisArgs {
    /// Clip id to animate.
    clip_id: String,
...
#[async_trait]
impl ToolHandler for PlanEmphasisTool {
        ...
        ToolSchema {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    "clip_id": {
                        "type": "string",
                        "description": "Clip id to animate."
                    },
                    "at_s": {
                        "type": "number",
                        "minimum": 0.0,
                        "description": "Clip-local emphasis start time."
                    },
                    "duration_s": {
                        "type": "number",
                        "minimum": 0.05,
                    ...
                    "objective": {
                        ...
                        "description": "Named emphasis job."
                    },
                    "target": {
                        "type": "string",
                        "enum": ["clip", "overlay", "title"],
                        "description": "Visual surface to animate."
                    },
                    "style": {
                        "type": "string",
                        "enum": ["snappy", "loose", "heavy", "neutral"],
                        "description": "LLM-friendly physics/timing intent."
                    },
                    "direction": {
                        "type": "string",
                        "enum": ["left", "right", "up", "down", "in", "out", "auto"],
                        "description": "Screen direction for slide or attention motion."
                    },
                    "intensity": {
                        "type": "number",
                        "minimum": 0.0,
                    ...
                    "beat_times_s": {
                        ...
                        "description": "Clip-local beat times. Emits a rhythm-driven pulse animation."
                    },
                    "every_nth_beat": {
                        "type": "integer",
                        "minimum": 1,
                        "description": "Use every Nth supplied beat."
                    },
                    "context": {
                        "type": "object",
                        "description": "Optional shot/visual context with subject_center, motion_direction, action_score, whip_pan_score, occlusion_score, saliency_map, or motion_vector."
                    }
                },
                "required": ["clip_id"]
            }),
    ...
    async fn handle(
        ...
        let args: PlanEmphasisArgs = serde_json::from_value(invocation.args).map_err(|e| {
            FunctionCallError::RespondToModel(format!(
                "plan_emphasis: invalid args ({e}). Required: clip_id."
            ))
        })?;
        if args.clip_id.trim().is_empty() {
            return Err(FunctionCallError::RespondToModel(
                "plan_emphasis: clip_id must be non-empty.".into(),
            ));
        }
        let plan = build_plan(&args)?;
        Ok(ToolOutput::text(plan.to_string()))
    }
}

fn build_plan(args: &PlanEmphasisArgs) -> Result<serde_json::Value, FunctionCallError> {
...
) -> Result<ParameterAnimation, FunctionCallError> {
...
fn beat_pulse_animation(
    ...
    if keyframes.len() < 2 {
        ...
        return Err(FunctionCallError::RespondToModel(
            "plan_emphasis: beat_times_s did not contain any usable beat times.".into(),
        ));
    }
    animation(
...
fn animations_edl(animations: &[ParameterAnimation]) -> Result<String, FunctionCallError> {
    ...
    for animation in animations {
        ...
        lines.push(format!(
            ...
            serde_json::to_string(animation).map_err(|e| {
                FunctionCallError::RespondToModel(format!(
                    "plan_emphasis: failed to serialize animation: {e}"
                ))
            })?
        ));
    }
...
mod tests {
    ...
    #[test]
    fn plan_emphasis_is_read_only() {
        assert!(!PlanEmphasisTool.is_mutating(&invoke(serde_json::json!({
            "clip_id": "clip-a"
        }))));
    }

    #[tokio::test]
    async fn returns_parseable_scale_punch_edl() {
        let out = PlanEmphasisTool
            .handle(
        ...
        assert_eq!(
            body.pointer("/recommended/motion").and_then(|v| v.as_str()),
            Some("scale_punch")
        );
        assert_eq!(
            body.pointer("/animations/0/target/parameter")
                .and_then(|v| v.as_str()),
            Some("overlay.scale")
        );
        let edl = body
            ...
            .and_then(|v| v.as_str())
            .unwrap();
        crate::edl::parse(edl).expect("emphasis EDL should parse");
    }

    #[tokio::test]
    async fn beat_times_emit_diminishing_keyframes() {
        let out = PlanEmphasisTool
            .handle(
                invoke(serde_json::json!({
                    "clip_id": "clip-a",
        ...
        let body: serde_json::Value = serde_json::from_str(&out.content).unwrap();
        let keyframes = body
            .pointer("/animations/0/keyframes")
            .and_then(|v| v.as_array())
            .unwrap();
        assert!(keyframes.len() >= 9);
        let first_peak = keyframes[1]["value"].as_f64().unwrap();
        let second_peak = keyframes[4]["value"].as_f64().unwrap();
        assert!(second_peak < first_peak);
    }

    #[tokio::test]
    async fn spring_style_keyframes_use_neutral_easing_metadata() {
        let out = PlanEmphasisTool
            .handle(
                invoke(serde_json::json!({
                    "clip_id": "clip-a",
            ...
            .unwrap();
        let body: serde_json::Value = serde_json::from_str(&out.content).unwrap();
        let keyframe = &body["animations"][0]["keyframes"][0];
        assert_eq!(keyframe["interpolation"], "spring");
        assert_eq!(keyframe["easing"], "linear");
    }

    #[tokio::test]
    async fn overlarge_every_nth_beat_reports_stride_not_bad_beats() {
        let err = PlanEmphasisTool
            .handle(
                invoke(serde_json::json!({
                    "clip_id": "clip-a",
                    "beat_times_s": [0.5, 1.0],
                    "every_nth_beat": 3
                })),
                fake_ctx(),
            )
        ...
        match err {
            FunctionCallError::RespondToModel(message) => {
                assert!(message.contains("every_nth_beat=3 skipped all 2 beats"));
            }
    ...
    #[tokio::test]
    async fn context_motion_direction_can_drive_slide_in() {
        let out = PlanEmphasisTool
            .handle(
                invoke(serde_json::json!({
                    "clip_id": "clip-a",
                    "objective": "direct_attention",
                    "direction": "auto",
                    "context": {
                        "visual_context": {
                            "subject_center": [0.35, 0.42],
        ...
        assert!(
            body.pointer("/missing_signals")
                .and_then(|v| v.as_array())
                .is_some_and(|signals| signals.iter().any(|value| value == "motion_vector"))
        );
    }
