//! `podcast_post_draft_check` tool — verify draft episode boundaries.
//!
...
/// Read-only post-draft boundary checker.
pub struct PodcastPostDraftCheckTool;

#[derive(Debug, Deserialize)]
struct PodcastPostDraftCheckArgs {
    #[serde(default)]
    boundary_window_s: Option<f64>,
}
...
#[async_trait]
impl ToolHandler for PodcastPostDraftCheckTool {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    "boundary_window_s": {
                        ...
                        "minimum": 15.0,
                        "maximum": 240.0,
                        "description": "Seconds to inspect at the start and end of the current draft timeline. Default 90."
                    }
                }
            }),
            ..Default::default()
    ...
    async fn handle(
        ...
        let args: PodcastPostDraftCheckArgs =
            serde_json::from_value(invocation.args).map_err(|e| {
                FunctionCallError::RespondToModel(format!(
                    "podcast_post_draft_check: invalid args ({e}). All fields are optional."
                ))
            })?;
        ...
        let project = Project::read(&ctx.project_root).map_err(|e| {
            FunctionCallError::RespondToModel(format!(
                "podcast_post_draft_check: failed to read project: {e}"
            ))
        })?;
        ...
        serde_json::to_string(&report)
            .map(ToolOutput::text)
            .map_err(|e| {
                FunctionCallError::Fatal(format!("podcast_post_draft_check serialize: {e}"))
            })
    }
}
...
mod tests {
    ...
    fn make_project(root: &std::path::Path, asset: &str) {
        ...
        clip.source_range = Some(TimeRange::new(
            ...
            RationalTime::new(120.0 * 24.0, 24.0),
        ));
    ...
    fn write_segments(root: &std::path::Path, asset: &str) {
        let path = root
            .join("index")
            .join("whisper")
        ...
        std::fs::write(
            ...
            serde_json::to_vec(&serde_json::json!({
                "data": {
                    "segments": [
                        {"text": "Welcome to the show.", "start_s": 0.0, "end_s": 3.0},
                        {"text": "This is the main interview.", "start_s": 20.0, "end_s": 25.0},
                        {"text": "Okay are we done? Cut.", "start_s": 110.0, "end_s": 115.0}
                    ]
                }
            }))
            .unwrap(),
    ...
    #[tokio::test]
    async fn detects_tail_chatter_in_current_draft() {
        let dir = tempfile::tempdir().unwrap();
        let asset = "raw/episode.mov";
        make_project(dir.path(), asset);
        write_segments(dir.path(), asset);

        let out = PodcastPostDraftCheckTool
            .handle(
                ToolInvocation {
                    call_id: "d1".into(),
                    name: "podcast_post_draft_check".into(),
                    args: serde_json::json!({"boundary_window_s": 30.0}),
                },
                ctx_at(dir.path()),
            )
            .await
        ...
        assert_eq!(value["requires_tighten_span"], true);
        assert_eq!(value["ending_check"]["issues"][0]["kind"], "tail_chatter");
    }
}
