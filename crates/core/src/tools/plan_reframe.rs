...
/// Read-only reframe planner.
pub struct PlanReframeTool;

#[derive(Debug, Deserialize)]
struct PlanReframeArgs {
    /// Clip id to reframe.
    clip_id: String,
    /// Delivery aspect ratio. Defaults to vertical 9:16.
    #[serde(default)]
    aspect_ratio: Option<String>,
    /// Source media width in pixels. When absent, assumes common 16:9 source.
    #[serde(default)]
    source_width: Option<u32>,
    /// Source media height in pixels. When absent, assumes common 16:9 source.
    #[serde(default)]
    source_height: Option<u32>,
    /// Optional normalized subject center.
    #[serde(default)]
    subject_center: Option<SubjectCenter>,
...
struct SubjectCenter {
    x: f64,
    y: f64,
}

#[async_trait]
impl ToolHandler for PlanReframeTool {
        ...
        ToolSchema {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    "clip_id": {
                        "type": "string",
                        "description": "Clip id to reframe. The returned EDL uses clip_uuid anchoring."
                    },
                    "aspect_ratio": {
                        "type": "string",
                        "enum": ["9:16", "1:1", "4:5", "16:9"],
                        "description": "Delivery aspect ratio. Defaults to 9:16."
                    },
                    "source_width": {
                        "type": "integer",
                        "minimum": 1,
                    ...
                    "subject_center": {
                        ...
                        "required": ["x", "y"],
                        "description": "Normalized subject center from vision evidence."
                    },
                    "context": {
                        "type": "object",
                        "description": "Optional vision context containing subject_center."
                    },
                    "safe_area": {
                        "type": "string",
                        "description": "Optional safe-area label, e.g. mobile."
                    },
                    "zoom": {
                        "type": "number",
                        "minimum": 1.0,
                        "maximum": 3.0,
                        "description": "Optional explicit zoom override."
                    }
                },
                "required": ["clip_id"]
            }),
    ...
    async fn handle(
        ...
        let args: PlanReframeArgs = serde_json::from_value(invocation.args).map_err(|e| {
            FunctionCallError::RespondToModel(format!(
                "plan_reframe: invalid args ({e}). Required: clip_id."
            ))
        })?;
...
fn build_plan(args: &PlanReframeArgs) -> Result<serde_json::Value, FunctionCallError> {
    let clip_id = args.clip_id.trim();
    if clip_id.is_empty() {
        return Err(FunctionCallError::RespondToModel(
            "plan_reframe: clip_id must be non-empty.".into(),
        ));
    }
    ...
    if !zoom.is_finite() || zoom < 1.0 {
        return Err(FunctionCallError::RespondToModel(
            "plan_reframe: zoom must be finite and >= 1.".into(),
        ));
    }
...
fn parse_ratio(raw: &str) -> Result<f64, FunctionCallError> {
...
fn validate_subject_center(center: SubjectCenter) -> Result<(), FunctionCallError> {
    if !center.x.is_finite() || !(0.0..=1.0).contains(&center.x) {
        return Err(FunctionCallError::RespondToModel(
            "plan_reframe: subject_center.x must be in 0..=1.".into(),
        ));
    }
    if !center.y.is_finite() || !(0.0..=1.0).contains(&center.y) {
        return Err(FunctionCallError::RespondToModel(
            "plan_reframe: subject_center.y must be in 0..=1.".into(),
        ));
    }
    Ok(())
}
