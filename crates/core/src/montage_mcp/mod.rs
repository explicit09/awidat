//! In-process Montage MCP server.
//!
//! Codex consumes this server like any other stdio MCP server (see the
//! `[mcp_servers.montage]` entry passed via `-c` in
...
//! our tools here keeps all Montage changes in `crates/`, not
...
//! [`crate::montage_mcp::tools`]; the rmcp-facing wrappers (with
...
//!    equivalent) in `crates/core/src/montage_mcp/tools/<name>.rs`,
...
use crate::montage_mcp::tools::analyze_sync::{self, AnalyzeSyncArgs};
use crate::montage_mcp::tools::apply_edl::{self, ApplyEdlArgs};
use crate::montage_mcp::tools::apply_episode_spans::{self, ApplyEpisodeSpansArgs};
use crate::montage_mcp::tools::assess_continuity::{self, AssessContinuityArgs};
use crate::montage_mcp::tools::assess_edit_quality::{self, AssessEditQualityArgs};
use crate::montage_mcp::tools::attempt_completion::{self, AttemptCompletionArgs};
use crate::montage_mcp::tools::broll_candidates::{self, BrollCandidatesArgs};
use crate::montage_mcp::tools::clip_search::{self, ClipSearchArgs};
use crate::montage_mcp::tools::color_scopes::{self, ColorScopesArgs};
use crate::montage_mcp::tools::create_stringout::{self, CreateStringoutArgs};
...
use crate::montage_mcp::tools::find_broll_opportunities::{self, FindBrollOpportunitiesArgs};
use crate::montage_mcp::tools::find_dead_air::{self, FindDeadAirArgs};
use crate::montage_mcp::tools::find_episode_start::{self, FindEpisodeStartArgs};
use crate::montage_mcp::tools::find_eye_contact::{self, FindEyeContactArgs};
use crate::montage_mcp::tools::find_false_starts::{self, FindFalseStartsArgs};
use crate::montage_mcp::tools::find_filler_words::{self, FindFillerWordsArgs};
use crate::montage_mcp::tools::find_generated_broll_opportunities::{
    self, FindGeneratedBrollOpportunitiesArgs,
};
use crate::montage_mcp::tools::find_moment::{self, FindMomentArgs};
use crate::montage_mcp::tools::find_speaker_oncam::{self, FindSpeakerOncamArgs};
use crate::montage_mcp::tools::granular_timeline::{
...
use crate::montage_mcp::tools::import_media::{self, ImportLocalArgs, ImportUrlArgs};
use crate::montage_mcp::tools::inspect_clip::{self, InspectClipArgs};
use crate::montage_mcp::tools::inspect_moment::{self, InspectMomentArgs};
use crate::montage_mcp::tools::list_assets::{self, ListAssetsArgs};
use crate::montage_mcp::tools::list_bins::{self, ListBinsArgs};
use crate::montage_mcp::tools::list_episodes::{self, ListEpisodesArgs};
...
use crate::montage_mcp::tools::plan_generated_media::{self, PlanGeneratedMediaArgs};
use crate::montage_mcp::tools::plan_look_regions::{self, PlanLookRegionsArgs};
use crate::montage_mcp::tools::plan_motion_scene::{self, PlanMotionSceneArgs};
use crate::montage_mcp::tools::plan_multicam::{self, PlanMulticamArgs};
use crate::montage_mcp::tools::plan_reframe::{self, PlanReframeArgs};
use crate::montage_mcp::tools::plan_scene_aware_short_form::{self, PlanSceneAwareShortFormArgs};
use crate::montage_mcp::tools::plan_short_form_review::{self, PlanShortFormReviewArgs};
use crate::montage_mcp::tools::plan_sound_design::{self, PlanSoundDesignArgs};
use crate::montage_mcp::tools::plan_split_edit::{self, PlanSplitEditArgs};
use crate::montage_mcp::tools::plan_transition::{self, PlanTransitionArgs};
use crate::montage_mcp::tools::plan_visual_support::{self, PlanVisualSupportArgs};
use crate::montage_mcp::tools::plan_visual_support_proposals::{
...
use crate::montage_mcp::tools::podcast_apply_accepted_edits::{
    self, PodcastApplyAcceptedEditsArgs,
};
use crate::montage_mcp::tools::podcast_audio_polish::{self, PodcastAudioPolishArgs};
use crate::montage_mcp::tools::podcast_cleanup_candidates::{self, PodcastCleanupCandidatesArgs};
use crate::montage_mcp::tools::podcast_edit_proposal::{self, PodcastEditProposalArgs};
use crate::montage_mcp::tools::podcast_editorial_review_pack::{
...
use crate::montage_mcp::tools::podcast_story_map::{self, PodcastStoryMapArgs};
use crate::montage_mcp::tools::podcast_visual_polish::{self, PodcastVisualPolishArgs};
use crate::montage_mcp::tools::poll_generated_media_job::{self, PollGeneratedMediaJobArgs};
use crate::montage_mcp::tools::poll_render::{self, PollRenderArgs};
use crate::montage_mcp::tools::preview_cache::{self, PreviewCacheArgs};
use crate::montage_mcp::tools::proxy_media::{self, GenerateProxyArgs};
use crate::montage_mcp::tools::read_broll_recommendations::{self, ReadBrollRecommendationsArgs};
use crate::montage_mcp::tools::read_index::{self, ReadIndexArgs};
use crate::montage_mcp::tools::read_media_intelligence::{self, ReadMediaIntelligenceArgs};
use crate::montage_mcp::tools::read_media_readiness::{self, ReadMediaReadinessArgs};
use crate::montage_mcp::tools::read_understanding::{self, ReadUnderstandingArgs};
...
use crate::montage_mcp::tools::stream_remux::{self, StreamRemuxArgs};
use crate::montage_mcp::tools::transcript_pack::{self, TranscriptPackArgs};
use crate::montage_mcp::tools::transcript_search::{self, TranscriptSearchArgs};
use crate::montage_mcp::tools::transition_context::{self, TransitionContextArgs};
use crate::montage_mcp::tools::update_plan::{self, UpdatePlanArgs};
use crate::montage_mcp::tools::use_broll::{self, UseBrollArgs};
use crate::montage_mcp::tools::use_generated_media::{self, UseGeneratedMediaArgs};
use crate::montage_mcp::tools::validate_transition_choice::{self, ValidateTransitionChoiceArgs};
use crate::montage_mcp::tools::vedit_blame::{self, VeditBlameArgs};
use crate::montage_mcp::tools::vedit_branch::{self, VeditBranchArgs};
use crate::montage_mcp::tools::vedit_changed_clip_ids::{self, VeditChangedClipIdsArgs};
use crate::montage_mcp::tools::vedit_checkout::{self, VeditCheckoutArgs};
use crate::montage_mcp::tools::vedit_commit::{self, VeditCommitArgs};
use crate::montage_mcp::tools::vedit_diff::{self, VeditDiffArgs};
use crate::montage_mcp::tools::vedit_log::{self, VeditLogArgs};
use crate::montage_mcp::tools::vedit_merge::{self, VeditMergeArgs};
use crate::montage_mcp::tools::vedit_merge_preflight::{self, VeditMergePreflightArgs};
use crate::montage_mcp::tools::vedit_revert::{self, VeditRevertArgs};
use crate::montage_mcp::tools::vedit_show::{self, VeditShowArgs};
use crate::montage_mcp::tools::vedit_tag::{self, VeditTagArgs};
...
/// macro on `impl MontageMcpServer`.
#[derive(Debug, Clone)]
pub struct MontageMcpServer {
...
impl Default for MontageMcpServer {
    fn default() -> Self {
        Self::new()
    }
}
...
stringouts in parallel (e.g. per arc, alt-cut, cold-open).",
    ...
    #[tool(
        description = "\
List durable episode spans stored in Timeline.metadata.montage.episodes as \
...
editorial-moments index is produced by `montage index --indexer \
...
destination. The tool selects an existing Montage export/package path, returns \
...
intent, and reusable reference assets to .montage/visual_support_defaults.json. \
...
`yt-dlp` on PATH.",
    ...
    /// caveat record under `.montage/yt_caveats.json`.
    #[tool(
        description = "\
...
root.",
    ...
    /// `generate_proxy` — generate or refresh the .montage proxy for
    /// one raw asset.
    ///
    ...
    #[tool(
        description = "\
Generate or refresh the .montage proxy for one raw asset. The proxy is \
the cached low-bitrate playback file under `<project>/.montage/proxies/`. \
...
if a stringout with that id already exists.",
    ...
    #[tool(
        description = "\
Persist reviewed episode spans into Timeline.metadata.montage.episodes. Use \
...
chapters, lower thirds, and captions.",
    ...
    /// under `.montage/proxies/`, `.montage/thumbnails/`, and
    /// `.montage/waveforms/`, and atomically updates the lifecycle
    /// manifest at `.montage/preview-cache/refresh-plan.json`.
    #[tool(
...
for hour-long timeline exports use the desktop UI or `montage render` \
CLI instead.",
        annotations(destructive_hint = true, read_only_hint = false)
    )]
...
instead.",
    ...
    /// per-asset sidecars under `<project>/.montage/index/`.
    #[tool(
        description = "\
...
view_episode shows they're missing.",
    ...
    /// under `<project>/.montage/generated-media/registry.json` and
...
`<project>/.montage/generated-media/registry.json` and returns whatever \
