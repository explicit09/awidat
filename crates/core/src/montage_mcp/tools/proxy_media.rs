//! `proxy_media` — generate the .montage proxy for one raw asset.
...
Generate or refresh the .montage proxy for one raw asset. \
The proxy is the cached low-bitrate playback file under \
`<project>/.montage/proxies/`. Pass `asset_id` as the project-relative \
