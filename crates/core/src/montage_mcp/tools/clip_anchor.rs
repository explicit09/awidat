...
//!   → emit THAT clip's own id (its `montage.clip_uuid`, unique per clip;
...
use montage_proto::otio::{Clip, MediaReference, StackChild, Timeline, TrackChild, TrackKind};
...
pub struct ResolvedAnchor {
    ...
    /// `montage.clip_uuid` (or its name) — never the asset path, so
...
///   clip's id (`montage.clip_uuid`, or its name) as the anchor — apply_edl
...
pub fn resolve_clip_anchor(
    ...
    // Emit the matched clip's OWN id (its `montage.clip_uuid`, unique per
...
/// The clip's emit-side anchor id: its `montage.clip_uuid` when present
...
fn load_project_timeline(project_root: &Path) -> Result<Timeline, String> {
    ...
    montage_proto::project::read_otio_timeline(&path, &mut warnings).map_err(|e| {
        format!(
...
/// The clip's `montage.clip_uuid` metadata string, if present. Mirrors
...
mod tests {
    ...
    fn single_clip_match_prefers_uuid_anchor() {
        // When the clip has an montage.clip_uuid, that unique id is emitted
        // (preferred over the name) so apply_edl resolves it unambiguously.
        let tmp = tempfile::tempdir().unwrap();
        let mut clip = external_clip("Interview", "raw/x.mp4");
        clip.metadata.montage = Some(montage_proto::montage_meta::MontageClipMetadata {
            extra: [(
                "clip_uuid".to_string(),
