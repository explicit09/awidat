//! `plan_visual_support` — read-only router for visual-support requests.
//! Ported from `crates/core/src/tools/plan_visual_support.rs` to the
...
/// Arguments to `plan_visual_support`.
#[derive(Debug, Default, Deserialize, Serialize, JsonSchema)]
pub struct PlanVisualSupportArgs {
    /// The user's visual-support request or the agent's proposed
    /// visual need.
    pub request: String,
}

/// Run `plan_visual_support`. The project root is unused — this
/// planner is pure text routing.
pub fn run(args: PlanVisualSupportArgs, _ctx: McpToolCtx) -> Result<String, String> {
    if args.request.trim().is_empty() {
        return Err("plan_visual_support: request must be non-empty.".into());
    }
    let route = route_visual_support_request(&args.request);
    serde_json::to_string_pretty(&route)
        .map_err(|e| format!("plan_visual_support: failed to serialize route: {e}"))
}
