...
use montage_proto::professional::{SelectDecision, SourceRange, SourceSelect, Stringout};
...
fn upsert_episode_stringout(
    meta: &mut montage_proto::montage_meta::MontageTimelineMetadata,
) -> usize {
...
Persist reviewed episode spans into Timeline.metadata.montage.episodes. \
...
mod tests {
    ...
    fn episode_tools_create_stringout_for_accepted_spans_only() {
        ...
        let meta = project.timeline.metadata.montage.unwrap();
        let stringout = meta
            .stringouts
            .iter()
