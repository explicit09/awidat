//! `color_scopes` — compute histogram, waveform, RGB parade, and
//! vectorscope evidence from a single video frame. Ported in step 5
//! from `crates/core/src/tools/color_scopes.rs` to the in-process
//! MCP server.
...
pub struct ColorScopesArgs {
    /// Project-relative asset id, OR an absolute path under the project.
    pub asset: String,
    /// Time in seconds to sample.
    pub t_s: f64,
    /// Scope resolution per axis. Defaults to 64.
    #[serde(default)]
    pub bins: Option<usize>,
    /// Maximum extracted frame width before scope computation. Defaults to 320.
    #[serde(default)]
...
/// Run `color_scopes` against the project resolved from
/// [`McpToolCtx`]. Returns the JSON body as `Ok(String)`; argument or
/// processing errors return `Err(String)`.
pub fn run(args: ColorScopesArgs, ctx: McpToolCtx) -> Result<String, String> {
    ...
    // The MCP tool is invoked from a sync context (rmcp dispatches the
    // async wrapper, but `run` itself is sync per the established
    // pattern). The original tool used async ffmpeg/ffprobe via
    // tokio::process; replicate that by running a small tokio runtime
    // for the blocking duration of this call.
    let runtime = tokio::runtime::Builder::new_current_thread()
        .enable_io()
        .enable_time()
        .build()
        .map_err(|e| format!("color_scopes: failed to build tokio runtime: {e}"))?;
    let frame = runtime.block_on(extract_rgb_frame(&asset_path, args.t_s, bins, max_width))?;
    let scopes = compute_color_scopes(&frame)
        .map_err(|e| format!("color_scopes: computation failed: {e}"))?;
...
/// scope rendering without going through the MCP dispatch indirection.
///
...
pub async fn extract_and_compute_color_scopes(
    ...
    let frame = extract_rgb_frame(asset_path, t_s, bins, max_width).await?;
...
pub(crate) async fn extract_rgb_frame(
    ...
    let ffmpeg = montage_render::ffmpeg_path()
        .map_err(|e| format!("color_scopes: ffmpeg unavailable: {e}"))?;
...
pub(crate) async fn probe_video_dimensions(asset_path: &Path) -> Result<(usize, usize), String> {
    let ffprobe = montage_render::ffprobe_path()
    ...
    let width = width
        .parse::<usize>()
        .map_err(|e| format!("color_scopes: invalid probed width: {e}"))?;
    let height = height
        .parse::<usize>()
        .map_err(|e| format!("color_scopes: invalid probed height: {e}"))?;
    if width == 0 || height == 0 {
        return Err("color_scopes: probed video dimensions must be positive".into());
    }
    Ok((width, height))
}
