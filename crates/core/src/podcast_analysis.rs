//! Podcast timeline QC analysis — shared business logic for the
//! `podcast_qc_report` and `start_render` tools.
//!
//! Shared by the legacy `ToolHandler` path and the live MCP path so
//! pre-render QC gating does not depend on which dispatch surface ran
//! the check.
...
pub fn is_podcast_project(project: &Project) -> bool {
    project
        .timeline
        .metadata
        .montage
...
pub fn build_podcast_qc_report(
...
mod tests {
    ...
    #[test]
    fn blocks_missing_media_and_reports_gaps() {
        let dir = tempfile::tempdir().unwrap();
        write_project(dir.path());
        let project = Project::read(dir.path()).unwrap();
        let value = build_podcast_qc_report(dir.path(), &project);
    ...
    #[test]
    fn blocks_primary_av_duration_mismatch() {
        let dir = tempfile::tempdir().unwrap();
    ...
    #[test]
    fn blocks_unverified_cleanup_hard_cut() {
        let dir = tempfile::tempdir().unwrap();
    ...
    #[test]
    fn transition_at_boundary_satisfies_smoothing_gate() {
    ...
    #[test]
    fn verified_cleanup_hard_cut_satisfies_smoothing_gate() {
        let dir = tempfile::tempdir().unwrap();
