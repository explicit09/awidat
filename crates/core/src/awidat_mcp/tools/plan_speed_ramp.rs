...
pub async fn run(args: PlanSpeedRampArgs, ctx: McpToolCtx) -> Result<String, String> {
    ...
    // Always probe the media — even when the trim duration is known — so an
    // offline/missing asset fails loud at PLAN time rather than producing a
    // "render-ready" EDL the render cannot use (finding 3365044139).
    let probed_s = probe_duration(&asset_path).await.ok_or_else(|| {
        format!(
            "plan_speed_ramp: could not probe a duration for {}; a ramp needs real, present source media",
            asset_path.display()
        )
    })?;
    // Plan over the CLIP's trimmed window when known (its source axis is
    // clip-relative); otherwise the whole media file's probed duration.
