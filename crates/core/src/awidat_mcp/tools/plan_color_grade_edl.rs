...
use awidat_effects::COLOR_PIPELINE;
...
/// `<space>_to_rec709_g24.csp` shaper ships with the awidat skills bundle.
...
/// Build the registry parameter map for an `awidat.color_pipeline` look.
...
/// Render a `Set Effect` `awidat.color_pipeline` EDL op block.
...
mod tests {
    ...
    use awidat_effects::{COLOR_CORRECTION, normalize_params};
