...
use awidat_render::{JobManager, OutputPathPolicy, RenderJobSpec, validate_render_output_path};
...
/// `StreamExportSpec` lives in `awidat_proto` and does not implement
...
pub struct StreamRemuxArgs {
    ...
    /// match the `StreamExportSpec` shape from `awidat_proto`:
