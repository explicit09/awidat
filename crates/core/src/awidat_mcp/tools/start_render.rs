...
use awidat_render::{OutputPathPolicy, RenderPlanLimitation, validate_render_output_path};
...
pub async fn run(args: StartRenderArgs, ctx: McpToolCtx) -> Result<String, String> {
    ...
    if has_master_loudnorm_plan {
        return Err(
            "start_render: this project has a master-loudnorm two-pass plan that the \
             ...
             the desktop UI or the `awidat render` CLI until out-of-process job state \
             is wired up."
                .into(),
        );
    }
    ...
    let ffmpeg_bin = awidat_render::ffmpeg_path()
...
fn build_start_render_manifest(
    ...
    let ffmpeg_path = awidat_render::ffmpeg_path()
        .map_err(|e| format!("start_render: failed to locate ffmpeg: {e}"))?;
    ...
    let sidecars = awidat_render::fingerprint_ffmpeg_subtitle_sidecars(input.argv)
        .map_err(|e| format!("start_render: failed to fingerprint render sidecars: {e}"))?;
    metadata.extend(
        awidat_render::ass_sidecar_layout_metadata(input.argv)
...
fn apply_preset_to_argv(
    ...
    let spec = awidat_render::RenderJobSpec {
        args: base,
...
turn — for hour-long timeline exports use the desktop UI or `awidat \
