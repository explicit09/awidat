//! Shared podcast cleanup scanners — dead air, filler words, and false
//! starts.
//!
//! Shared by the legacy `ToolHandler` path (`tools::find_dead_air`,
//! `tools::find_filler_words`, `tools::find_false_starts`) and the
//! live MCP path so the podcast cleanup/proposal tools on both
//! dispatch surfaces see identical findings.
                ...
                TrackChild::Clip(clip) => {
                    ...
                    if let Some(words) = load_whisper_words(project_root, &asset_id) {
                        for w in &words {
                            ...
                            let normalized =
                                crate::transcript_cleanup::normalize_transcript_token(&w.text);
        ...
        for tc in &track.children {
            match tc {
                TrackChild::Clip(clip) => {
                    ...
                    if let Some(words) =
                        load_whisper_words_for_false_starts(project_root, &asset_id)
                    {
...
fn load_whisper_words_for_false_starts(
    project_root: &Path,
    asset_id: &str,
...
#[cfg(test)]
mod filler_words_tests {
    use super::*;
    use crate::transcript_cleanup::default_filler_tokens;
    use montage_proto::otio::{
        Clip, ExternalReference, MediaReference, RationalTime, Stack, StackChild, TimeRange, Track,
        TrackChild, TrackKind,
    };
    ...
    fn finds_default_fillers_inside_clip_range() {
        ...
        let fillers = default_filler_tokens(false);
    ...
    fn aggressive_includes_discourse_markers() {
        ...
        let aggro = default_filler_tokens(true);
    ...
    fn override_filler_list_replaces_default() {
        ...
        let custom: Vec<String> = vec!["foobar".into()];
