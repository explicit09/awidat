...
//! `montage agent <agent@montage.local>`. These tests pin the new
//! contract:
//!
...
//!      the "montage agent" default — backward compat.
//!
...
fn legacy_commit_path_stamps_resolved_default_author() {
    ...
    // chain falls back to the "montage agent" default. Should the host
    // happen to have MONTAGE_USER_* set (CI runner, dev shell), we
    ...
    match (
        std::env::var("MONTAGE_USER_NAME").ok(),
        std::env::var("MONTAGE_USER_EMAIL").ok(),
    ) {
        ...
        _ => {
            ...
            assert_eq!(entry.author.name, "montage agent");
            assert_eq!(entry.author.email, "agent@montage.local");
        }
    }
}
