//! Montage-specific TUI surface.
//!
//! Step 6 of the codex-harness migration mounts an Montage panel into
//! codex's chat layout. The panel auto-mounts when the
//! `MONTAGE_PROJECT_ROOT` env var is set (the same env that drives the
//! Montage MCP server in `crates/core/src/montage_mcp/`). When unset,
//! the panel is `None` and codex renders exactly as upstream — this
//! keeps the fork edit invisible to non-Montage users.
//!
...
//! refresh via [`MontagePanel::refresh`], which the App calls when an
//! MCP tool result indicates a destructive Montage tool succeeded.
...
/// Env var that picks the project root for the Montage panel and the
/// in-process MCP server. Matches
/// `montage_core::montage_mcp::context::PROJECT_ROOT_ENV` — but we
/// copy the literal here so the codex TUI fork stays independent of
/// the montage-core crate (it depends only on `montage_proto`).
...
/// Montage side panel. Owns the two view widgets and the project
...
impl MontagePanel {
    /// Construct an Montage panel. Always-visible per the step-6
    /// design: when `MONTAGE_PROJECT_ROOT` is set, use it; otherwise
    ...
    /// `montage_core::montage_mcp::context::McpToolCtx::resolve`.
