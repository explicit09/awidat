...
#[tokio::test]
#[serial(codex_auth_env)]
async fn refresh_request_fails_without_configured_montage_oauth_client_id() {
    let _client_id_guard = EnvVarGuard::remove(MONTAGE_OAUTH_CLIENT_ID_ENV_VAR);
    let _override_guard = EnvVarGuard::remove(CLIENT_ID_OVERRIDE_ENV_VAR);

    let client = HttpClient::new(reqwest::Client::new());
    let error = request_chatgpt_token_refresh("old-refresh-token".to_string(), &client)
        .await
        .expect_err("refresh should fail without a configured OAuth client id");

    match error {
        RefreshTokenError::Transient(err) => {
            assert!(err.to_string().contains(MONTAGE_OAUTH_CLIENT_ID_ENV_VAR));
        }
        other => panic!("expected a transient configuration error, got {other:?}"),
    }
}
