...
- 2026-07-16: R25 CLOSED at root (4 rounds): flake = headless Chromium rasterizing the video surface layer stale in CDP screenshots (decoder verifiably correct). Fix: draw the in-page-verified decoded frame into a content-layer canvas and hide the video for capture. Run 29478049212: all 5 cases SSIM=1.000000 attempt 1. Full diagnostic layers retained (content check + re-seek, fresh-browser evidence-preserving retry, named failure modes).
- 2026-07-16: WAVE 4 DONE — 4a (seams + 12 route tests, R8/R16; bearer_auth fail-closed) and 4b (R15 sqlite_store deleted, legacy ToolRegistry infra deleted, net -2266 lines). Remaining new items: R26/R27/R28 (small social-server fixes found by the tests).
