...
Montage's performance targets are manual workflows, not normal development or CI checks. Use the benchmark-specific evidence requirements below when results need to be reproducible or comparable.
...
Each wall sample starts before the helper process is spawned and ends when it exits, so it includes process and helper setup, decoder provenance lookup, Tokio runtime construction, the production `generate_waveform` call, correctness hashing, JSON serialization, and the atomic helper-result write. The production call dominates this wrapper work for the two-hour fixture, but short smoke runs should be interpreted as end-to-end helper overhead rather than decoder-only timing. Helpers run in isolated process groups; any monitor failure terminates and reaps the helper plus decoder descendants before returning. The unique timestamped JSON report records generated UTC time, fixture-generator and helper-decoder provenance, executable/Cargo.lock/source hashes, Rust toolchain details, raw wall time, aggregate helper-plus-FFmpeg peak RSS, and maximum cumulative live-tree CPU time, with median/p95/MAD summaries; the recursive sampler targets 10 ms and rejects a run if any observed gap exceeds 100 ms. Disk-I/O accounting remains intentionally unavailable, so use platform tooling beside the report when required.

The Make target defaults builds to this checkout's `target/` directory and fixture work and evidence to `$TMPDIR` (or `/tmp`). Set `CARGO_TARGET_DIR`, `MONTAGE_WAVEFORM_PERF_WORK_DIR`, and `MONTAGE_WAVEFORM_PERF_EVIDENCE_DIR` explicitly when an external drive is intended. For a short internal APFS smoke:
...
`make perf-audio-energy` and `make perf-clip-lifecycle` are manual, heavyweight performance workflows. Each target performs one release build of `montage-index-perf` in the caller's `CARGO_TARGET_DIR` (or this checkout's `target/`), then gives its controller that exact executable.

The audio-energy controller runs the real dispatcher/indexer path against a deterministic mixed-signal M4A fixture. Its full default is a one-hour fixture, one warmup, and five timed samples, so run it manually only when that cost is intended. It needs the already prepared `audio-energy-mcp` Python environment, `uv`, `ffmpeg`, and `ffprobe`. It records end-to-end wall time (including dispatcher/process setup), sampled process-tree peak RSS, temporary-directory high water, and median/p95/MAD summaries. It rejects malformed or empty audio output and requires every timed canonical audio-energy payload to exactly match the warmup. Fixture-cache publication is serialized per duration and recovers an interrupted one-file publication, while a corrupt complete media/metadata pair still fails closed. Each dispatcher runs in an isolated process group; cleanup keeps its leader unreaped through group quiescence checks and any TERM/KILL signals, while sampled PIDs remain diagnostics rather than signal targets. A successful run writes a unique JSON evidence report below its work root's `evidence/` directory, retaining per-sample work and provenance for the binary, fixture files, complete Python workspace (including its ignored venv), runtime, tools, Git state, machine, and filesystems. Those execution inputs are snapshotted before and after the measured sequence, and any change rejects the run.

The audio controller resolves the dispatcher's bundled `uv` sidecar first; only when that sidecar is absent can `MONTAGE_UV=/absolute/path/to/uv`, `PATH`, or the `~/.local/bin/uv` fallback select `uv`. Preflight and dispatcher samples both bind `uv` to `python/.venv`, run offline without syncing, disable configuration discovery and Python downloads, clear inherited `UV_*`, `PYTHON*`, and `VIRTUAL_ENV` controls, and set deterministic hash, locale, and time controls. The report records resolved module paths and hashes for the audio-energy package and its NumPy, SciPy, and pyloudnorm dependencies.

The CLIP lifecycle controller measures the real six-asset dispatcher path: one warmup and seven timed samples by default. Before timing, it independently probes each fixture's duration and runs the production `fps=0.5,scale=224:224` FFmpeg filter to establish an exact frame-count oracle; every sidecar must match that count and the canonical two-second timestamp grid. It also validates asset fingerprints, exact `ViT-B-32/openai` model identity, 512-dimensional float16 embeddings, and stable semantic output against the warmup while excluding volatile timestamps and per-run timing fields from that comparison. Preflight separately verifies the imported `clip_mcp` model constants and pinned artifact. The controller removes inherited Python code paths, disables user-site imports and bytecode writes, preserves the provenanced `uv`/FFmpeg tool directories at the front of child `PATH`, rejects tracked, untracked, or unsafe ignored source inputs, and content-manifests the complete supplied Python workspace—including the ignored venv—before and after the measured sequence. It also rehashes the dispatcher, controller Python, six asset contents, model blob, and tool binaries after fixture observation and after the measured samples. A successful dispatcher exit is accepted only if its process group disappears naturally; forced cleanup still runs for hygiene but fails the sample. Cleanup is intentionally limited to the dispatcher process group created with `start_new_session`; the controller never signals an observed bare PID because PID reuse cannot be made race-free on macOS. Any observed descendant that remains outside that group fails the run and is reported for inspection. Production dispatcher, MCP, and FFmpeg descendants must retain their inherited group; uncontained custom binaries are outside the benchmark contract. Its report also records sampled process-tree peak RSS, tool/runtime/model/source/filesystem provenance, and retained sample logs. Both controllers reject work or evidence locations inside the protected Python workspace or the repository checkout, preventing the benchmark from mutating an input that its final provenance snapshot must match. Both reports are performance evidence only when the fixture/model/machine/workspace facts in their provenance are comparable.
