...
| --- | --- | --- |
| First-class auto-reframe agent surface | Implemented as `plan_reframe`, a read-only planner that returns an `awidat.reframe` EDL fragment for `apply_edl`. | `crates/core/src/tools/plan_reframe.rs`; registered in CLI, TUI, and desktop registries; exposed by `skills/short-form/SKILL.md`. |
| Reframe mutation path | Uses the established `apply_edl` boundary via `*** Set Effect` rather than a second mutating tool. | `awidat.reframe` is validated by `awidat-effects`; render reads it as `ReframePlan`. |
...
- vertical 9:16 planning emits an `apply_edl`-ready `awidat.reframe` fragment,
