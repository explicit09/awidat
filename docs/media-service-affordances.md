...
- Proxy status: Awidat already writes proxy mp4 files under `.awidat/proxies/` and exposes a lifecycle report with fresh, stale, missing, orphan, and pending states through the desktop transcode command.
- Waveform extraction: Awidat already writes per-asset waveform sidecars under `.awidat/waveforms/` and the desktop timeline cache lazily reads them.
...
- Export/package presets: Awidat already has durable `DeliveryProfile` and `ExportPreset` contracts in `crates/proto` and lowering/preflight helpers in `crates/render`.
- Composition verification: Awidat already generates composition regions and render-side composition graph diagnostics. `composition-mcp` now includes a lightweight `verification` report for generated regions so malformed ranges, missing sources, and bad confidence values are visible in the existing sidecar.

## Deferred

- Post-composition thumbnail rendering like the reference `ThumbnailCandidates` should wait until Awidat has a stable composition preview artifact to score.
