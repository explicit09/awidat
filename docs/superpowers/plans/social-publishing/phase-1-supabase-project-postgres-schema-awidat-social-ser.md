# Phase 1: Supabase project + Postgres schema + awidat-social service deployment shape
...
- [USER] Create the Fly.io org + app (fly apps create awidat-social) and a deploy token; or Railway equivalent.
...
- [WE-STAGE] Author and commit the migrations, PgSocialStore + tests, the awidat-social-server crate, Dockerfile, fly.toml, runbook, and CI changes.
...
## Phase 1 — Supabase project, Postgres schema, and the `awidat-social` service deployment shape
...
4. A deployable `awidat-social-server` binary (an Axum wrapper over the framework-neutral `SocialApi`) plus its deployment shape (where it runs, how Supabase invokes it, how it reaches Postgres securely), resolving the three Phase-1 open questions.
...
**Q: How `awidat-social` is invoked from Supabase — HTTP (pg_net/Edge Function) vs. a queue it polls?**
...
2. Create the Fly.io org + app (`fly apps create awidat-social`) and a Fly auth token for CI/deploy. (Or Railway equivalent.)
...
- The `awidat-social-server` binary crate (Axum) + Dockerfile + `fly.toml` + env-var contract, authored and committed; secrets injected by the user at deploy time.
...
- Register a **disabled/no-op-safe placeholder** cron entry that documents the contract but does nothing destructive: a commented-out `cron.schedule('awidat-publish-tick', '* * * * *', $$ select net.http_post(...) $$)` template with the service URL and auth header as placeholders. The actual enabling happens in Phase 4; Phase 1 only proves the extensions install and the template is correct. Rationale: keeps Phase 1 strictly infra; avoids a live cron hitting an endpoint that has no real adapter yet.
...
#### Step 3 — Add Postgres support to the `awidat-social` crate
- Edit `crates/social/Cargo.toml` (**per D1 — NOT async sqlx**): add the **synchronous** `postgres` crate + `r2d2` + `r2d2_postgres` (new workspace deps; absent from Cargo.lock today — add them) for the blocking connection pool the sync store uses. Do NOT add async `sqlx` to `awidat-social`. Keep `rusqlite` — the SQLite store stays for the desktop's local/editing use and as the reference for tests.
...
- Verification: `cargo build -p awidat-social` compiles with the new deps.
...
- New file `crates/social/tests/pg_store.rs` (integration test, gated behind an env var like `AWIDAT_TEST_PG_URL` so CI without Postgres still passes; locally point it at `supabase db` or a `docker run postgres`).
...
- Verification command: `AWIDAT_TEST_PG_URL=postgres://... cargo test -p awidat-social --test pg_store`. Also run `cargo test -p awidat-social` (no env var) to confirm the existing SQLite/in-memory suite is still green and untouched.

#### Step 6 — Create the `awidat-social-server` binary crate (Axum wrapper, mock adapters)
- New crate `crates/social-server/` (per D2 — this is THE single server crate every later phase means; add to workspace `members`). Depends on `awidat-social`, `tokio`, `reqwest` (for provider HTTP later), `tracing`/`tracing-subscriber`, and `axum` **with crate-local features `["http1","tokio","json"]`** (per G1 — the workspace `axum` is `default-features=false` and won't serve otherwise). The store deps (`postgres`/`r2d2`) come via `awidat-social`. No async `sqlx`.
...
- Verification: `cargo build -p awidat-social-server`; a local run with `DATABASE_URL` pointing at local Postgres responds 200 on `/health` and `/providers` (curl); `POST /internal/tick` with the secret returns a drained-count, without the secret returns 401.

#### Step 7 — Containerize and define the deployment shape
- `crates/social-server/Dockerfile`: multi-stage Rust build (cargo build --release -p awidat-social-server) → slim runtime image; expose `BIND_ADDR` port; copy the `crates/social/migrations/` directory so the boot-time migrator finds the SQL files at runtime.
- `crates/social-server/fly.toml`: app name `awidat-social`, one shared-cpu-1x machine, internal port matching `BIND_ADDR`, a `[[services]]` http handler on 443, and an `[checks]` HTTP healthcheck hitting `/health`. Region set to match the Supabase project region (user fills in).
...
- Edit `.github/workflows/ci.yml`: add `cargo build -p awidat-social-server` to the build matrix and run `cargo test -p awidat-social` (existing fast suite). Optionally add a Postgres service container so the `pg_store` integration test runs in CI with `AWIDAT_TEST_PG_URL` set. Do not put any secrets in CI for the live Supabase project.
