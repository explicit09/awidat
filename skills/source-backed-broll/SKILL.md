...
5. If no asset exists, use generation tools only after the editorial moment is
   ...
   `start_generated_media_job`. Use `min(15, max(4, ceil(duration_s)))`
   for generation and pass the original accepted timeline `duration_s` into
   `use_generated_media`, trimming the generated clip to fit the selected span.
